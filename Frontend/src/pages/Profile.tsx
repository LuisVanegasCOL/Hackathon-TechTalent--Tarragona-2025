...
import { getPedidosByUser } from '@/lib/api';
...
const Profile: React.FC = () => {
  ...
  const fetchUserOrders = async () => {
    try {
      ...
      const response = await getPedidosByUser(user!.id);
      
      if (response.success && response.pedidos) {
        // Transformar formato del backend al frontend
        const userOrders = response.pedidos.map((pedido: any) => {
          // Calcular total basado en los detalles del pedido
          const total = pedido.detalles?.reduce((sum: number, detalle: any) => {
            return sum + (parseFloat(detalle.precio || '0') * detalle.cantidad);
          }, 0) || 0;

          // Transformar detalles a items para el frontend
          const items = pedido.detalles?.map((detalle: any) => ({
            id: detalle.id_producto.toString(),
            name: detalle.producto_nombre,
            price: parseFloat(detalle.precio || '0'),
            quantity: detalle.cantidad,
            category: detalle.tipo,
          ...
          return {
            ...
            date: pedido.fecha_pedido || new Date().toISOString(),
