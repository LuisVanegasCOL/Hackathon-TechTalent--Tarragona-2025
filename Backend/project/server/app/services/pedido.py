...
def create_pedido_service(pedido_data):
    ...
    try:
        ...
        db.commit()
        cursor.close()
        return {'success': True, 'id': pedido_id, 'message': 'Pedido creado correctamente'}
...
def get_pedidos_by_user_service(user_id):
    ...
    try:
        ...
        for pedido in pedidos:
            cursor.execute("""
                SELECT dp.id_producto, dp.cantidad, pr.nombre as producto_nombre, pr.tipo, pr.precio
                FROM detalle_pedido dp
                JOIN producto pr ON dp.id_producto = pr.id
                WHERE dp.id_pedido = %s
            """, (pedido['id'],))
        ...
        logging.error(f"Error al obtener pedidos del usuario {user_id}: {e}")
